...
# Copy and secure the start script
sudo cp "$SOURCE_SCRIPT" "$DEST_SCRIPT"
sudo chmod +x "$DEST_SCRIPT"
sudo chown root:root "$DEST_SCRIPT"

# Copy and secure the stop script
SOURCE_STOP="$HOME/Git/ThinkPad/hotspot/stop-hotspot.sh"
DEST_STOP="$DEST_DIR/stop-hotspot.sh"

echo "[+] Deploying stop script..."
sudo cp "$SOURCE_STOP" "$DEST_STOP"
sudo chmod +x "$DEST_STOP"
sudo chown root:root "$DEST_STOP"
